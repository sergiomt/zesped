YUI.add('gallery-divbox', function(Y) {
	...
	Y.extend(LB, Y.Base, {
	        ...
	        objBody.append(create('<div id="lightbox"></div>')
				.append(create('<div id="outerImageContainer"></div>')
					.append(create('<div id="imageContainer"></div>')
						.append(create('<div id="lightboxImage"></div>'))
		...
		start: function (selectedLink) {
			...
			if (selectedLink.get("rel") === LIGHTBOX) {
				...
				imageArray.push([selectedLink.get("href"), ""]);
			} else {
		...
		_changeImage: function (imageNum) {
			...
			*/
		    ...
		    YUI().use("io-form", "transition", function(Y) {
		      function completedModalFormLoad(id, o, args) {
		    	lbi.get("lightboxImage").set("innerHTML", o.responseText);
		    	var s = document.getElementById("lightboxImage").childNodes.item(0).style;
		    	if (s==undefined) s = document.getElementById("lightboxImage").childNodes.item(1).style;
		    	var w, h, p;
		    	p = s.width.indexOf("px");
		    	w = Number(p<0 ? s.width : s.width.substring(0,p));
		    	p = s.height.indexOf("px");
		    	h = Number(p<0 ? s.height : s.height.substring(0,p));
		    	if (!isNaN(w) && !isNaN(h))
		    	  lbi._resizeImageContainer(w, h);
		    	  onLoadLightbox();
		      };
		       
		      var e = eval (httpRequestText("SessionPing.action"));
		      if (e.length>0) 
		    	document.location = "enter.jsp?e="+e;
		      else
		        Y.io(lbi.get(IMAGE_ARRAY)[imageNum][0], { method:'GET', on: {'complete':completedModalFormLoad} });
		    });		      
