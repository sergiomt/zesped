...
/**
  * Move ComboBox Selection to a given value decimal
  * @param objCombo HTML <SELECT> Object
  * @param idValue Value to find inside ComboBox 
*/
function setComboDecimal (objCombo, idValue) {
  ...
  if (idValue.length==0) {
	  ...
	  var len = opt.length;
	  var rgx = "";
	  for (var c=0; c<idValue.length; c++)
	    if (idValue.charAt(c)=='.' || idValue.charAt(c)==',')
		  rgx += "[\\x2C|\\x2E]";
	    else
	      rgx += idValue.charAt(c);
	  
	  rgx = new RegExp (rgx);
	  for (var i=0;i<len;i++)
	    if (opt[i].value.match(rgx)) {
		    opt.selectedIndex = i;
		    break;
	    } // fi()	  
  }
